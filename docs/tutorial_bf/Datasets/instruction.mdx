...
We construct instruction dataset for 2D/3D modality instruction tuning via GPT API.

## 2D Instruction Datasets

2D instruction datasets are build on [MS-COCO](https://cocodataset.org), [Bamboo](https://github.com/ZhangYuanhan-AI/Bamboo), [Locount](https://isrc.iscas.ac.cn/gitlab/research/locount-dataset) and [TextVQA](https://textvqa.org) dataset. You can download them from [here](https://opendatalab.com/LAMM/LAMM/tree/main/raw/2D_Instruct).
...
## 3D Instruction Datasets

We provide two 3D instruction datasets, "Scan2Inst" and "LAMM3D-Dataset", for 3D instruction tuning.
